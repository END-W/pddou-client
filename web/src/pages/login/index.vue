  ...
  <div id="login">
    ...
    <div class="login-container">
      <div class="login-header logo">
        ...
        <p class="title">票兜兜电影</p>
        <i class="eng-title">PDDou Movie</i>
      </div>
...
<script>
import { getPhoneCode, phoneLogin, pwdLogin } from '@/api/user'
  ...
  methods: {
    ...
    async getValidateCode() {
      if (this.checkPhone) {
        ...
        let data = await getPhoneCode({ phone: this.phone })
        if (data.code === 20000) {
          // 获取手机验证码成功
          MessageBox.alert('手机验证码为：' + data.message)
        } else {
          // 获取手机验证码失败
    ...
    login() {
      ...
      if (this.loginType) {
        ...
        if (this.phone === '') {
          ...
          phoneLogin({ phone: this.phone, code: this.phoneCode })
            .then(response => {
              Toast({
                message: '登录成功',
                position: 'middle',
        ...
        if (this.userName === '') {
          ...
          pwdLogin({ userName: this.userName, password: this.password })
            .then(response => {
              Toast({
                message: '登录成功',
                position: 'middle',
